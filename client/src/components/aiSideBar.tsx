...
const AiSideBar = ({ currentFile }) => {
   ...
   return (
      <div className="max-w-2xl mx-auto p-4 space-y-4">
         <div className="h-170 overflow-y-auto p-3 rounded-lg space-y-2 shadow-md flex flex-col">
            {messages.map((msg) => (
               <div key={msg.id} className={`p-2 rounded-lg max-w-xs ${msg.sender === "user" ? "bg-[#D8D9DA] text-black self-start" : "bg-[#0F4C75] text-white self-end"}`}>
                  {msg.text}
               </div>
            ))}
         </div>
